...
def graphTemplate(ax, norm_CC, norm_N):
    ...
    par1.tick_params(axis='y', labelcolor='burlywood', length=0)
...
def animation(k, ax, t, XC, XE, N, W, CC_printed):

    if k==0:
        ax.axhline(y=CC_printed, color='orange', linestyle='--', label = "Carrying Capacity")

    if k==1:
        ax.legend(loc='upper left', bbox_to_anchor=(-0.2, -0.065),
        fancybox=True, shadow=True, ncol=5, fontsize=2)
